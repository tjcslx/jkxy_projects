...
<body>
        ...
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <?php
                    if (connectToDb()) {
                        $mysqli = connectToDb();
                        $query = "SELECT n1.n_id, n1.n_heading, CONCAT(LEFT(n1.n_content, 10), '…') AS n_abstract, n1.insert_time, do.dno_name FROM news_main n1, d_news_origin do WHERE n1.n_origin = do.dno_id AND n1.n_status = '1' ORDER BY n1.n_id DESC";
                        ...
                        if ($num = 0) {
                            ...
                            while ($row = $res -> fetch_assoc()) {
                                echo "<tr><td><a href=\"#\" data-toggle=\"modal\" data-target=\"#newsmodal\" data-whatever=\"".$row["n_id"]."\">".$row["n_id"]."</a></td><td><a href=\"#\" data-toggle=\"modal\" data-target=\"#newsmodal\" data-whatever=\"".$row["n_id"]."\">".$row["n_heading"]."</a></td><td><a href=\"#\" data-toggle=\"modal\" data-target=\"#newsmodal\" data-whatever=\"".$row["n_id"]."\">".$row["n_abstract"]."</a></td><td><a href=\"#\" data-toggle=\"modal\" data-target=\"#newsmodal\" data-whatever=\"".$row["n_id"]."\">".$row["dno_name"]."</a></td><td><a href=\"#\" data-toggle=\"modal\" data-target=\"#newsmodal\" data-whatever=\"".$row["n_id"]."\">".$row["insert_time"]."</a></td></tr>";
                            }
                            $res -> free();
                        };
                    } else {
    ...
    <script src="./js/backend.build.js"></script>
</body>
